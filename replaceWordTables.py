#!/usr/bin/env python3
"""
Sync custom table styles from one Word document into another file or folder.

Usage:
    python sync_table_styles.py source.docx target.docx
    python sync_table_styles.py source.docx /path/to/folder
    python sync_table_styles.py -b source.docx /path/to/folder

Behavior:
- Reads all custom table styles from the source document.
- Removes all existing custom table styles from each target document.
- Inserts the source custom table styles into each target document.
- Overwrites target files in-place unless -b/--backup is used.

Notes:
- This script only touches styles in word/styles.xml.
- It preserves all non-table styles.
- It preserves built-in table styles unless they are marked as custom table styles.
- Supports .docx and .docm files.
"""
...
def replace_custom_table_styles_in_doc(
    ...
    with zipfile.ZipFile(target_path, "r") as zin:
        ...
        root = ET.fromstring(styles_xml)
        removed_count = remove_custom_table_styles(root)
    ...
    return removed_count, len(source_styles)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "a target file or every Word document in a target folder."
        )
    )
    parser.add_argument(
        "source",
...
def main() -> int:
    ...
    for doc in target_files:
        try:
            removed, inserted = replace_custom_table_styles_in_doc(
